...
Durable execution for AI agents that need to maintain state over extended periods, coordinate multi-step LLM tool calls with retries, and support human intervention. Temporal handles the reliability and state management so developers focus on agent logic.

### Alerting & Incident Response
Event-driven alert routing, notification delivery with escalation logic, and incident lifecycle management.

## When NOT to Use Temporal

| Scenario | Why Not | Better Alternative |
|----------|---------|-------------------|
| Simple request/response APIs | Unnecessary overhead if no failure recovery needed | Plain HTTP/gRPC |
| Sub-millisecond latency | Temporal adds ~10-50ms | Direct service calls |
| Pure data streaming | Not designed for continuous streams | Kafka, Flink, Pulsar |
...
**Note:** Temporal IS a good fit for background jobs that need reliability, retries, or take seconds+. Especially in Python where blocking/CPU-bound operations benefit from Temporal's activity executor model (ThreadPoolExecutor/ProcessPoolExecutor) with automatic retries, heartbeating, and timeout management.
